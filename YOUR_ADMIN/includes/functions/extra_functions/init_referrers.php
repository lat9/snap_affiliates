...
$currentDescription = SNAP_MODULE_UPDATE_DATE . ', <a href="http://vinosdefrutastropicales.com" target="_blank" rel="noreferrer">Vinos de Frutas Tropicales</a>';
...
//
if (!defined('SNAP_MODULE_VERSION') || SNAP_MODULE_VERSION !== $currentVersion) {
    /*----
    ...
    */
    ...
    $installedVersion = (defined('SNAP_MODULE_VERSION')) ? SNAP_MODULE_VERSION : '0';
  
    foreach ($snap_config_items as $config_item) {
        if ($installedVersion < $config_item['version'] && !defined($config_item['key'])) {
    ...
    $db->Execute("UPDATE " . TABLE_CONFIGURATION . " SET configuration_value = '$currentVersion', configuration_description = '$currentDescription', last_modified=now() WHERE configuration_group_id = $configuration_group_id AND configuration_key = 'SNAP_MODULE_VERSION' LIMIT 1");
